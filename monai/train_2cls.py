...
#     stats = torch.tensor(params.stats)
    ...
    pos_weight = torch.tensor([0.43])
    loss_function = torch.nn.BCEWithLogitsLoss(pos_weight=pos_weight.to(device))
    ...
    for epoch in range(args.epoch):
        ...
        writer.add_scalar("running/learning_rate", scheduler.get_lr()[0])
        epoch_loss /= step
        ...
        if (epoch + 1) % val_interval == 0:
            ...
            with torch.no_grad():
                ...
                valid_loss = loss_function(y_pred.float(), y.float())
